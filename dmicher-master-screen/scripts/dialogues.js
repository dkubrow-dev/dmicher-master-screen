...
export function getDialogueContext(sceneId, dialogueId) {
  const scene = game.scenes.get(sceneId), runtime = scene ? getRuntime(scene) : null;
...
export function createDialogueService({ emitEvent, onChange = () => {}, context = getDialogueContext,
  ...
  const process = async (command, user, commandId) => {
    if ((command.schemeId ?? "main") !== "main") fail("Эта схема взаимодействия не поддерживается.");
    const initial = context(command.sceneId, command.dialogueId);
    if (!initial.scene) fail("Сцена не найдена.");
    const events = [];
    const result = await lock(initial.scene, async () => {
      const state = clone(runtimeOf(initial.scene));
      ...
      const emit = (name, session, payload, suffix = "response") => {
        if (!/^[a-z][a-z0-9_.-]{0,99}$/.test(name)) fail("Некорректное имя события взаимодействия.");
        ...
        if (command.kind === "start") {
          ...
          if (!(session?.status === "active" && session.runId === state.runId && session.dialogueId === dialogue.id)) {
            ...
            session = { sessionId: random(), userId: user.id, actorTokenId: command.actorTokenId, dialogueId: dialogue.id,
              schemeId: "main", runId: state.runId, nodeId: dialogue.startNodeId, step: 0, status: "active" };
    ...
    for (const event of events) {
      try {
        ...
        await lock(initial.scene, async () => {
          const state = clone(runtimeOf(initial.scene)); state.error = result.error;
  ...
  const send = async (raw) => {
    if (raw.schemeId !== undefined && raw.schemeId !== "main") fail("Эта схема взаимодействия не поддерживается.");
    const command = { ...raw, schemeId: "main" };
  ...
  return Object.freeze({
    ...
    getContext: context,
    requestStart: (command) => send({ ...command, kind: "start", runId: context(command.sceneId, command.dialogueId).runtime?.runId }),
    requestInteraction: (command) => send({ ...command, kind: "interaction", runId: context(command.sceneId).runtime?.runId }),
