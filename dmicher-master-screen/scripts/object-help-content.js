    ...
    "sections": [
      {
        ...
        "bodyRu": "В «Инструменты → Магазины» создайте магазин, задайте имя, арт, вид ассортимента и подтверждение мастера. Перетащите предметы Foundry в «На старте», укажите количество, нажмите «На старте → Текущие» и «Сохранить», чтобы начать торговлю этим набором. В «Автоматизация → Магазины» добавьте магазин объекту и настройте доступные состояния и условия игрока. Один магазин можно открыть с нескольких объектов; все они разделяют текущий запас и одну торговую сессию.",
        "bodyEn": "In Tools → Shops, create a shop with a name, art, stock display and GM approval. Drop Foundry items into At start, set quantities, choose At start → Current and Save to trade with this assortment. In Automation → Shops, attach it to an object and configure available states and player conditions. Several objects can open one shop; they share its current stock and single trading session."
      },
      {
        "id": "inventory",
        "ru": "Подготовить и изменить запасы",
        "en": "Prepare and edit stock",
        "bodyRu": "«На старте» хранит подготовленный ассортимент для первого открытия магазина и ручного или настроенного возврата. Просмотр карты, открытие ширмы, перезагрузка страницы и обычный запуск или продолжение автоматизации не пополняют магазин. Автоматический возврат при активации сцены или новом входе в состояние включается отдельно в назначении магазина. «Текущие» показывает реальные товары: исполняющий мастер может менять количество, удалять их или добавлять перетаскиванием. «На старте → Текущие» готовит сброс, «Текущие → На старте» сохраняет нынешний ассортимент как подготовку. Обе кнопки меняют черновик: примените «Сохранить» или «Отменить ввод». Сохранение правок текущих товаров и любой явный сброс отменяют незавершённые сделки; завершённые обмены и предметы персонажей не откатываются. Если запас успел измениться во время правки, черновик остаётся: «Обновить текущие товары» перечитает только текущий список, сохранив остальные поля.",
        "bodyEn": "At start stores the prepared stock for the shop's first opening and manual or configured restoration. Viewing the map, opening the screen, reloading, or normally starting or resuming automation does not replenish a shop. Automatic restoration on scene activation or new state entry is enabled separately in the shop assignment. Current shows the live items: the authority GM may edit quantities, remove entries or drop new items. At start → Current prepares a reset; Current → At start adopts the live assortment as preparation. Both buttons change the draft: choose Save or Discard edits. Saving current stock changes or any explicit reset cancels unfinished trades; completed exchanges and character items are not rolled back. If stock changes while you edit it, the draft remains: Reload current stock rereads only the live list and preserves other fields."
      },
      {
        "id": "dialogue",
        "ru": "Собрать разговор",
...
export const OBJECT_HELP_SETTINGS = [
  ...
  { id: "settings-objects", ru: "Автоматизация объектов", en: "Object automation", fields: [
    ...
    ["feature-macro", "feature-macro", "Макрос условия", "Condition macro", "В блоке «Условия запуска». Дополнительная проверка для самостоятельного запуска игроком. Может учитывать переменные объекта и состояние; пустое поле не добавляет ограничения. Явный вызов инструментов скриптом мастера от этой проверки не зависит.", "Inside Launch conditions. An additional check for player-initiated access. It can use the object's variables and state; an empty field adds no restriction. An explicit GM script call is independent of this check."],
    ["shop-restoration-activation", "shop-restoration", "Возврат магазина: активация", "Shop restoration: activation", "«Вручную» — бесплатный возврат из каталога магазинов. Premium позволяет пополнять магазин при новом входе в назначенное состояние или штатной активации карты для игроков. Просмотр карты, перезагрузка и продолжение остановленного состояния не пополняют магазин. Сброс отменяет незавершённую сделку, но не отнимает уже переданные вещи. Один магазин у нескольких объектов имеет общие остатки.", "Manually restores stock for free from the shop catalog. Premium can restore it on new entry into an assigned state or native scene activation for players. Viewing, reloading or resuming a stopped state does not restock it. Restoration cancels unfinished trades but never removes items already transferred. Objects sharing a shop share its stock."],
    ["shop-restoration-macro", "shop-restoration-macro", "Макрос условия возврата", "Restoration condition macro", "Дополнительная проверка только для автоматического возврата. true разрешает пополнение, false отменяет; пустое поле разрешает. Использует переменные объекта и выбранное состояние. Не зависит от условий запуска игроком. Без Premium сохранённая настройка не исполняется.", "An additional check for automatic restoration only. true allows restocking, false prevents it; an empty field allows it. Uses the object's variables and selected state. Independent of player launch conditions. Saved automation does not execute without Premium."]
  ] },
  { id: "settings-tool-catalog", ru: "Каталоги инструментов", en: "Tool catalogs", fields: [
    ...
    ["shopStock", "stock", "Количество на старте", "At-start quantity", "Целое неотрицательное количество для ручного или настроенного возврата. Сохранение этого списка само по себе не меняет текущие товары. Перед первой торговлей скопируйте подготовленный набор в «Текущие» и сохраните.", "A non-negative whole quantity for manual or configured restoration. Saving this list alone does not change current stock. Before the first trade, copy the prepared assortment to Current and save."],
    ["shopCurrentStock", "current-stock", "Текущее количество", "Current quantity", "Целое неотрицательное количество реально доступных товаров. Меняет исполняющий мастер. Сохранение изменений отменяет незавершённые сделки; завершённые обмены не откатываются.", "A non-negative whole quantity of currently available items. Only the authority GM may edit it. Saving changes cancels unfinished trades; completed exchanges are not rolled back."],
    ['[data-screen-action="copyShopInventory"][data-stock-zone="current"]', "reset-stock", "На старте → Текущие", "At start → Current", "Копирует стартовые товары в текущий черновик. После «Сохранить» заменяет текущий список и отменяет незавершённые сделки, даже если количество не изменилось.", "Copies at-start items into the current draft. Save replaces live stock and cancels unfinished trades, even when quantities are unchanged."],
    ['[data-screen-action="copyShopInventory"][data-stock-zone="initial"]', "adopt-stock", "Текущие → На старте", "Current → At start", "Копирует текущие товары в стартовую подготовку. Применяется после «Сохранить»; не пополняет магазин и не отменяет сделки.", "Copies current items into the at-start preparation. Applies after Save; it does not replenish the shop or cancel trades."],
