...
export function normalizeObjectBinding(raw) {
  ...
  return { type: raw.type, id: raw.id, schemeId: raw.schemeId ?? null, tags: normalizeTags(raw.tags), notes: raw.notes ?? "",
...
export function getObjectBindings(scene) {
  ...
  return { ...stored, bindings: { ...bindings, ...stored.bindings } };
}
...
export function validateObjectBinding(scene, binding, definitions = getDefinitions(scene)) {
  ...
  if (!binding.schemeId && (binding.shop || binding.dialogue || binding.features.length || Object.keys(binding.episodes).length)) fail("Сначала назначьте объект схеме.");
...
export function reconcileDefinitionBindings(scene, previous, definitions) {
  ...
  for (const binding of Object.values(next.bindings)) {
    ...
    const definition = definitions.find((entry) => entry.schemeId === binding.schemeId);
    if (!definition) { binding.schemeId = null; binding.shop = null; binding.dialogue = null; binding.features = []; binding.legacyVariants = []; binding.episodes = {}; binding.entry = transition(); changed = true; continue; }
...
export function importObjectConfiguration(scene, source, { schemeId, episodeMapping = new Map(), definitions, eventCatalog, sourceEventCatalog } = {}) {
  ...
  for (const [key, raw] of Object.entries(incoming.bindings)) {
    ...
    if (existing?.schemeId === schemeId) {
      ...
      binding.entry = clone(existing.entry); binding.tags = clone(existing.tags); binding.notes = existing.notes;
...
export class SceneObjects {
  ...
  save(descriptor, patch, { expectedRevision, allowReassign = false } = {}) {
    return withSceneLock(this.scene, async () => {
      ...
      const next = normalizeObjectBinding({ ...previous, ...clone(patch), ...descriptor });
      ...
      if (changingOwner && (previous?.schemeId || previous?.conflictingSchemeIds?.length)) {
        for (const [field, fallback] of Object.entries({ shop: null, dialogue: null, features: [], episodes: {}, entry: transition() })) if (!Object.hasOwn(patch, field)) next[field] = fallback;
      }
      if (next.legacyVariants) {
        next.legacyVariants = next.legacyVariants.filter((entry) => !changingOwner && !Object.hasOwn(patch, entry.kind));
      }
      if (!next.schemeId) { next.shop = null; next.dialogue = null; next.features = []; next.episodes = {}; next.entry = transition(); }
  ...
  remove(descriptor, options = {}) { return this.save(descriptor, { schemeId: null }, { ...options, allowReassign: true }); }
}

function resolved(scene, descriptor, context, kind) {
  const binding = getObjectBindings(scene).bindings[objectKey(descriptor)];
  if (!binding || !binding.schemeId || binding.schemeId !== context.schemeId) return null;
...
export function materializeEpisode(scene, definition, source) {
  ...
  for (const binding of Object.values(bindings)) if (binding.conflictingSchemeIds?.includes(definition.schemeId)) fail(`Объект ${objectKey(binding)} настроен в нескольких схемах. Назначьте ему одного владельца.`);
  ...
  for (const binding of Object.values(bindings)) {
    if (binding.schemeId !== definition.schemeId || binding.conflictingSchemeIds?.length) continue;
    ...
    for (const feature of binding.features) {
      ...
      if (feature.kind === "patrol") { episode.tokens[target.id] ??= defaultTokenBehavior(); episode.tokens[target.id].patrol = clone(feature.patrol); }
