...
import { exportCatalogDependencies, mergeCatalogDependencies, builtinCatalog } from "./event-catalog.js";
...
export class SchemeEditor {
  ...
  transferEpisode(from, to, episodeId, { copy = true, name } = {}) { return this.change((definitions) => {
    ...
    const owned = Object.values(getObjectBindings(this.scene).bindings).some((binding) => binding.schemeId === from && (
      ...
      || binding.features.some(scoped) || binding.routines.some((entry) => entry.episodeId === episodeId)));
    ...
    if (!copy) { source.episodes = source.episodes.filter((entry) => entry.id !== episodeId); }
    return next;
  }); }
  ...
  exportEpisode(schemeId, id) {
    ...
    return { format: MODULE_ID, kind: "episode", version: 1, schemeId, data: clone(data), ...objects, catalog: combineCatalogs(exportCatalogDependencies(this.scene, [data]), objects.catalog) };
  }
  importScheme(envelope, { name } = {}) {
    ...
    return this.change((definitions) => {
      ...
      const names = new Set([...definitions.eventCatalog.events, ...builtinCatalog().events].map((entry) => entry.name));
      for (const episode of next.episodes) for (const event of episode.events) if (!names.has(event)) throw new Error(`В JSON отсутствует зависимое событие «${event}».`);
  ...
  importEpisode(schemeId, envelope, { name } = {}) {
    ...
    if (typeof envelope.schemeId !== "string" || !/^[A-Za-z0-9_-]{1,64}$/.test(envelope.schemeId)) throw new Error("В JSON эпизода отсутствует ID исходной схемы.");
    const source = normalizeDefinition({ ...defaultDefinition(), entryEpisodeId: envelope.data?.id, episodes: [envelope.data] }).episodes[0];
    return this.change((definitions) => {
      ...
      remapScope([next], envelope.schemeId, schemeId, new Map([[source.id, next.id]]));
      definitions.eventCatalog = mergeCatalogDependencies(this.scene, portableCatalog(envelope.catalog, envelope.schemeId, schemeId));
      destination.episodes.push(next);
