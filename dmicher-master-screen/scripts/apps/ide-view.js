...
export const TAB_LABELS = Object.freeze({ scene: "Сцена", events: "События", macros: "Макросы", other: "Иное", parameters: "Параметры", reference: "Подсказка" });
export const OTHER_BLOCKS = Object.freeze([
  { id: "tokens", name: "НИП и магазины", mode: "constructor" },
  { id: "tags", name: "Теги объектов", mode: "constructor" },
  { id: "dialogues", name: "Диалоги и взаимодействия", mode: "constructor" },
  ...
  { id: "manual", name: "Магазины и ручные диалоги", mode: "director" }
]);
...
export function renderEventTree(catalog, selection, mode) {
  ...
  for (const event of catalog.events) {
    ...
    rows.push(`<tr role="row" aria-level="1" aria-selected="${selected}" class="${selected ? "is-selected" : ""}"><td><button class="ms-tree-name" type="button" data-screen-action="selectNode" data-kind="event" data-id="${esc(event.id)}">◇ ${esc(event.name)}</button></td><td>${event.builtin ? "Встроенное" : ""}</td></tr>`);
    for (const trigger of catalog.triggers.filter((item) => item.eventId === event.id)) {
      ...
      rows.push(`<tr role="row" aria-level="2" aria-selected="${chosen}" class="${chosen ? "is-selected" : ""}"><td><button class="ms-tree-name ms-tree-child" type="button" data-screen-action="selectNode" data-kind="trigger" data-id="${esc(trigger.id)}">↳ ${esc(trigger.name)}</button></td><td>${trigger.parameters.length} пол.</td></tr>`);
    }
  }
