...
export class DialogueCatalogApplication extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {
    ...
    const episode = context.definition.episodes.find((entry) => entry.id === this.episodeId) ?? context.definition.episodes[0];
    ...
    return { ...parent, sceneName: context.scene.name, sceneId: this.sceneId,
      episodeId: this.episodeId, episodes: context.definition.episodes,
  ...
  async _onRender(context, options) {
    ...
    this.element.addEventListener("change", (event) => {
      if (event.target.name === "catalogEpisode") {
  ...
  async handleAction(action, data = {}) {
    ...
    const descriptor = { sceneId: this.sceneId, episodeId: this.episodeId, dialogueId: this.dialogueId };
