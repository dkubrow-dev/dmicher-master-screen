...
export class DialogueCatalogApplication extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {
    ...
    return { ...parent, sceneName: context.scene.name, sceneId: this.sceneId,
      ...
      actions: (context.episode?.interactions ?? []).map((entry) => ({ ...entry, targetName: (entry.target.type === "Token" ? context.scene.tokens.get(entry.target.id) : context.scene.tiles?.get(entry.target.id))?.name ?? entry.target.id })), subscriptions: context.episode?.subscriptions ?? [],
