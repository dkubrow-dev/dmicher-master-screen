...
export function openGroupSubscription(scene, groupId, id, onSaved) {
  ...
  const editor = new ScriptBlockEditor({ title:t("Подписка группы", "Group subscription"),
    ...
    save: async script => { getDefinition(scene,{groupId}); await catalog.saveSubscription({...draft,handler:"script",macroUuid:"",script},{expectedRevision:revision}); onSaved?.(); }
  });
