...
export const SCREEN_HELP_PAGES = [
  ...
  page("director", "Вести сцену", "Run the scene", [
    ...
    section("journal", "Проверить последние реакции", "Review recent reactions", "В дополнительной зоне Режиссёра откройте «Консоль»: видны создатель сигнала, принявшие его подписчики, результаты и ошибки. Раскройте строку для параметров и идентификаторов. «Отладка» добавляет шаги скриптов, переходы, магазины и диалоги. Журнал хранит последние 500 записей этого подключения и показывает записи текущей сцены; перезагрузка или «Очистить» убирает их. При прокрутке вверх новые записи не сдвигают чтение. При нескольких мастерах смотрите консоль того, кто выполняет автоматизацию.", "Open Console in Director's secondary area to see each signal's emitter, accepting subscribers, results and errors. Expand a row for parameters and identifiers. Debug adds script steps, transitions, shops and dialogues. The log keeps the latest 500 records from this connection and shows the current scene's entries; reloading or Clear removes them. Scrolling up keeps your reading position while new records arrive. With multiple GMs, use the console of the GM running automation.")
  ]),
  page("stop", "Остановить автоматизацию", "Stop automation", [
    ...
    section("resume", "Продолжить в выбранных состояниях", "Continue in the selected states", "«Продолжить всё» заново запускает текущие выбранные состояния всех групп: скрипты перехода выполняются сначала. Ручное отключение автоматизации отдельного объекта сохраняется.", "Resume all restarts every group's currently selected state: state-entry scripts run from the beginning. An object's manual automation disabling remains in effect."),
  ...
  page("dialogues", "Поговорить с объектом", "Talk to an object", [
    section("history", "Прочитать переписку", "Read the conversation", "Реплики объекта и выбранные вами ответы остаются в одном окне. Новые ответы доступны после последней реплики; прежние сообщения можно прокрутить вверх. В блоке диалога выберите выравнивание картинки: слева или справа. Текст обтекает картинку. В вашем ответе портрет персонажа всегда справа.", "Object messages and your selected replies remain in one window. Available replies follow the latest message; scroll up to read earlier messages. In a dialogue block, choose left or right image alignment. Text wraps around the image. Your character's portrait is always on the right of your reply."),
    section("listen", "Присоединиться слушателем", "Join as a listener", "Значок 💬 над персонажем показывает, что он ведёт разговор. Подойдите своим персонажем в пределах 5 единиц сцены и прямой видимости, кликните участника и выберите «Слушать диалог». Вы увидите историю и новые сообщения. Ответы выбирает начавший разговор игрок. Закрытие вашего окна не прерывает его диалог.", "A 💬 marker above a character means they are speaking. Approach with your character within 5 scene units and line of sight, click the speaker and select Listen to dialogue. You can read the history and new messages. The player who started the conversation chooses replies. Closing your window does not interrupt their dialogue."),
...
export const SCREEN_HELP_SETTINGS = [
...

  {
    ...
    fields: [["[data-screen-debug]", "debug", "Отладка", "Debug",
      "В Режиссёре откройте дополнительную вкладку «Консоль», включите «Отладка» и повторите проблемное действие. Появятся переходы, шаги скриптов, операции магазинов и диалогов. Без галки остаются сигналы, результаты подписчиков и ошибки. Настройка общая для мира, выключена по умолчанию и применяется сразу. Снятие галки скрывает подробные записи без удаления. Они также доступны в консоли Foundry (F12 → Console, уровень Verbose/Debug, фильтр dmicher-master-screen). При нескольких мастерах исполнение видно у мастера, который выполняет автоматизацию.",
      "In Director, open the secondary Console tab, enable Debug, and repeat the affected action. It adds transitions, script steps, shop and dialogue operations. Without Debug, signals, subscriber results and errors remain visible. This world-wide setting is off by default and applies immediately. Disabling it hides detailed entries without deleting them. They are also available in the Foundry console (F12 → Console, Verbose/Debug level, dmicher-master-screen filter). With multiple GMs, execution appears for the GM running automation."]]
  },
