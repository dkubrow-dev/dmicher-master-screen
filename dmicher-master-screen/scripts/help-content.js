...
export const SCREEN_HELP_PAGES = [
  ...
  page("stop", "Остановить автоматизацию", "Stop automation", [
    section("group-controls", "Управлять одной группой", "Control one group", "В Режиссёре откройте «Сцена». Справа в строке каждой группы доступны ▶ «Запустить группу», ■ «Остановить группу» и ↶ «Вернуть группу в исходное состояние». Запуск начинает заново выбранное состояние, а для новой группы — состояние входа. Восстановление выполняет исходные скрипты только этой группы и выбирает состояние входа, оставляя её остановленной. Другие группы продолжают работать. Эти команды также доступны в параметрах выбранной группы и записываются в Консоль.", "In Director, open Scene. Each group row has ▶ Start group, ■ Stop group and ↶ Restore group initial state on the right. Start restarts the selected state, or the entry state for a new group. Restore runs only that group's initial scripts and selects its entry state, leaving it stopped. Other groups keep running. These commands are also available in the selected group's parameters and are recorded in the Console."),
