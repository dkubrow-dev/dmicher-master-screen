...
export const SCREEN_HELP_PAGES = [
  ...
  page("stop", "Остановить автоматизацию", "Stop automation", [
    ...
    section("restore", "Вернуть подготовленное начало сцены", "Restore the prepared scene setup", "Нажмите «Вернуть в исходное состояние» один раз: после подготовки групп выполняются настроенные исходные скрипты объектов с заданными в них длительностями. Группы возвращаются к состояниям входа. Автоматизация остаётся остановленной; проверьте сцену перед «Продолжить всё». Во время подготовки и выполнения доступна кнопка «Остановить всё». Предметы, запасы магазинов и другие потраченные ресурсы не восстанавливаются автоматически.", "Click Restore initial state once: after group preparation, the objects' configured initial scripts run with their configured durations. Groups return to their entry states. Automation stays stopped; inspect the scene before Resume all. Stop all remains available during preparation and execution. Items, shop stock and other spent resources are not automatically restored.")
  ]),
