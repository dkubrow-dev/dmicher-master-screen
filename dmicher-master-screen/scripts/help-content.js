...
export const SCREEN_HELP_PAGES = [
  ...
  page("constructor", "Организовать подготовку", "Organize preparation", [
    section("sorting", "Расположить строки удобно для себя", "Arrange rows for yourself", "В основной зоне перетащите строку за значок ⠿ выше или ниже соседней строки того же уровня. Ветвь дерева переносится вместе с содержимым. Можно выделить значок клавишей Tab и использовать Alt + ↑/↓. Порядок личный: сохраняется для вашей учётной записи и сцены в этом браузере, общий для Конструктора и Режиссёра. Перестановка не меняет подготовку и порядок исполнения скриптов. Перенос состояния в другую группу в Конструкторе по-прежнему требует отдельного выбора копирования или перемещения.", "In the main area, drag a row by its ⠿ grip above or below a sibling at the same level. A tree branch moves with its contents. You can also focus the grip with Tab and use Alt + Up/Down. The order is personal: saved for your account and scene in this browser and shared by Constructor and Director. Reordering does not change preparation or script execution. Moving a state to another group in Constructor still asks whether to copy or move it."),
    ...
    section("tabs", "Выбрать инструмент", "Choose a tool", "«Инструменты» содержат магазины, диалоги, окна и заметки. «Автоматизация» содержит сигналы и макросы. Категории только раскрывают меню. Стрелки прокручивают длинный ряд вкладок. «Иное» содержит оставшиеся настройки карты и прямых взаимодействий.", "Tools contains shops, dialogues, windows and notes. Automation contains signals and macros. Categories only expand their menu. Arrows scroll overflowing tabs. Other contains remaining map and direct interaction settings.")
  ]),
  page("signals", "Связать действия сигналами", "Connect actions with signals", [
    ...
    section("subscribers", "Подключить событие объекта", "Connect an object event", "Сначала включите нужный сигнал в «Автоматизация → Свойства → Сигналы» его источника. По умолчанию сигналы объектов выключены. У получателя добавьте подписку, затем подготовьте скрипт в «Поведение → События». Подписка также может вызывать прикреплённый макрос. Проверка при сохранении показывает несовместимые параметры. Подписку можно отключить, изменить или удалить.", "First enable the intended signal in its source's Automation → Properties → Signals. Object signals start disabled. Add a subscription on the recipient, then prepare its script in Behavior → Events. A subscription can also call an attached macro. Save-time validation reports incompatible parameters. Subscriptions can be disabled, edited or removed."),
  ...
  page("macros", "Подключить готовый макрос", "Attach a prepared macro", [
    section("prepare", "Назначить макрос объекту", "Assign a macro to an object", "Создайте или исправьте макрос в штатном окне Foundry и прикрепите его через «Автоматизация → Свойства → Макросы». Отдельный шаг «Макрос» требует Premium. Условия меню и макросы переходов бесплатны и редактируются в своей настройке. Для события или реакции выберите входные параметры и откройте «Шаблон», если нужен образец.", "Create or edit a macro in Foundry's native window and attach it through Automation → Properties → Macros. The separate Macro step requires Premium. Menu conditions and transition macros are free and edited in their own settings. For an event or reaction, select its input parameters and open Template when you need an example."),
  ...
  page("npc", "Оживить объекты сцены", "Bring scene objects to life", [section("behavior", "Подготовить поведение", "Prepare behavior", 'В Конструкторе кликните объект и выберите «Автоматизация». Задайте исходное состояние и переходы, затем рутину, события и реакции. Магазины и диалоги доступны для токенов, тайлов, рисунков и регионов на собственных вкладках. Подробнее: <a data-help-page="scripts">скрипты</a> и <a data-help-page="objects">объекты</a>.', 'In Constructor, click an object and select Automation. Prepare its initial state and transitions, then routines, events and reactions. Tokens, tiles, drawings and regions have their own Shops and Dialogues tabs. See <a data-help-page="scripts">scripts</a> and <a data-help-page="objects">objects</a>.')]),
  ...
  page("stop", "Остановить автоматизацию", "Stop automation", [
    section("group-controls", "Управлять одной группой", "Control one group", "В Режиссёре откройте «Сцена». Справа в строке каждой группы доступны ▶ «Запустить группу», ■ «Остановить группу» и ↶ «Вернуть группу в исходное состояние». После остановки запуск применяет настройки прерывания скриптов; новая группа начинает со состояния входа. Восстановление отменяет команды объектов группы и ожидающие замены, снимает их ручное отключение, выполняет исходные скрипты и выбирает состояние входа. Группа остаётся остановленной до запуска; другие группы продолжают работать. Эти команды также доступны в параметрах выбранной группы и записываются в Консоль.", "In Director, open Scene. Each group row has ▶ Start group, ■ Stop group and ↶ Restore group initial state on the right. After a stop, Start applies script interruption settings; a new group begins at its entry state. Restore cancels the group's object commands and pending replacements, clears their manual automation disables, runs their initial scripts and selects the entry state. The group stays stopped until you start it; other groups keep running. These commands are also available in the selected group's parameters and are recorded in the Console."),
    ...
    section("restore", "Вернуть подготовленное начало сцены", "Restore the prepared scene setup", "Нажмите «Вернуть в исходное состояние» один раз. Прежние команды объектов и ожидающие замены отменяются; ручное отключение поведения снимается. Настроенные исходные скрипты выполняются с заданными длительностями, группы возвращаются к состояниям входа. Группы остаются остановленными: проверьте сцену и нажмите «Продолжить всё», чтобы запустить подготовку заново. Во время восстановления доступна кнопка «Остановить всё». Предметы, запасы магазинов и другие потраченные ресурсы не восстанавливаются автоматически.", "Click Restore initial state once. Previous object commands and pending replacements are cancelled; manual behavior disables are cleared. Configured initial scripts run with their set durations, and groups return to their entry states. Groups stay stopped: inspect the scene and select Resume all to start the prepared behavior afresh. Stop all remains available during restoration. Items, shop stock and other spent resources are not automatically restored.")
  ]),
  ...
  page("conditions", "Ограничить взаимодействия", "Limit interactions", [section("access", "Допустить нужных персонажей", "Admit the intended characters", "Настройте активность, состояния, дальность и теги у назначения инструмента объекту. Пустой разрешающий список допускает всех; запрещающий тег исключает персонажа. Для диалога бесплатный макрос условия может дополнительно разрешить запуск по переменным и текущему состоянию. «Показывать в меню, если недоступен» сохраняет отключённый пункт магазина или диалога. Лимит общий для назначения, по умолчанию один запуск; продолжение сессии не расходует его повторно. Сброс при входе настраивается отдельно.", "Set activity, states, range and tags on the object's tool assignment. An empty allow list accepts all; a denied tag excludes a character. A dialogue's free condition macro can additionally check variables and the current state. Show in menu when unavailable retains a disabled shop or dialogue entry. The assignment has a shared limit, one activation by default; resuming a session does not consume it again. Reset on entry is configured separately.")]),
  page("workspace", "Подготовить рабочие окна", "Prepare workspace windows", [section("windows", "Показать нужные документы", "Show useful documents", "Расставьте нужные окна и сохраните конфигурацию в «Инструменты → Окна». Её можно применить из Ширмы или шагом «Окна». Для каждого окна настройте закрытие при смене конфигурации; перемещение вручную не меняет активную конфигурацию. Аналогично сохраните набор объектов карты в «Инструменты → Заметки». Общий вид окон меняется в Generics.", "Arrange the needed windows and save a configuration in Tools → Windows. Apply it from Master screen or a Windows step. Set whether each window closes when configurations change; manual movement does not change the active configuration. Similarly save map notes in Tools → Notes. Shared window appearance is configured in Generics.")]),
  ...
  },
  {
    ...
    "fields": [
      ...
      [
        ...
        "Громкость диалогов",
        "Dialogue volume",
        "В «Плейлисты → Управление громкостью» ползунок диалогов всегда виден; без лицензии он отключён. С Premium меняйте громкость для себя. Ноль отключает звук; громкость других участников не меняется. Прежнее значение сохраняется при потере лицензии.",
        "Under Playlists → Volume controls, the dialogue slider is always visible; without a license it is disabled. With Premium, adjust your own volume. Zero mutes audio; other participants are unaffected. The saved value survives loss of the license."
      ],
    ]
  },
  {
