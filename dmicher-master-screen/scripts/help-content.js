...
export const SCREEN_HELP_PAGES = [
  ...
  page("dialogues", "Поговорить с объектом", "Talk to an object", [
    ...
    section("history", "Прочитать переписку", "Read the conversation", "По умолчанию разговор идёт в чате Foundry; мастер может выбрать отдельное окно. Переписка сохраняет реплики объекта и принятые ответы персонажа. Короткие варианты ответа показаны кнопками; длинные — списком с выбором и кнопкой «Ответить». Только ведущий игрок выбирает ответ. Картинка блока может быть слева или справа; портрет в ответе персонажа всегда справа.", "Conversations use Foundry chat by default; the GM can choose a separate window. The transcript retains object lines and accepted character replies. Short choices use buttons; longer choices use a selection list and Reply. Only the leading player chooses replies. A page image may be left or right; the character's reply portrait is always on the right."),
