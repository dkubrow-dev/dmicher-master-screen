...
export const SCREEN_HELP_PAGES = [
  ...
  page("signals", "Связать действия сигналами", "Connect actions with signals", [
    section("create", "Выбрать источник", "Choose a source", "На вкладке «Сигналы» раскройте группу, объект-эмитент и его сигналы. Объекты без группы показаны внизу. Выберите сигнал для редактирования. Пользовательскому сигналу задайте понятное имя и описание; имя не должно повторяться у этого эмитента.", "On Signals, expand a group, an emitter object and its signals. Ungrouped objects appear at the bottom. Select a signal to edit it. Give a custom signal a meaningful name and description; its name must be unique for that emitter."),
    section("fields", "Описать данные и результат", "Describe data and results", "Параметры передают сведения о действии; возвраты собирают ответы подписчиков. Добавьте поля нужного типа, ограничения и значения по умолчанию. Встроенные сигналы и их системные поля нельзя менять, но можно добавлять собственные поля. Для автоматически вызываемых сигналов задавайте новым полям значение по умолчанию.", "Parameters describe the action; returns collect subscriber responses. Add typed fields, limits and defaults. Built-in signals and system fields cannot be changed, but custom fields can be added. Give extra fields defaults when the signal is emitted automatically."),
...
,
  { id: "settings-signals", ru: "Сигналы и подписки", en: "Signals and subscriptions", fields: [
    ...
    ["field-default", "default", "Значение по умолчанию", "Default value", "Подставляется при отсутствии входящего значения. Вводите JSON: текст в кавычках, числа без кавычек, true, false или null.", "Used when an input value is absent. Enter JSON: quoted text, unquoted numbers, true, false or null."],
