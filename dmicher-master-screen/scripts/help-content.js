...
export const SCREEN_HELP_PAGES = [
  ...
  page("stop", "Остановить автоматизацию", "Stop automation", [
    ...
    section("restore", "Вернуть подготовленное начало сцены", "Restore the prepared scene setup", "Нажмите «Вернуть в исходное состояние» один раз. Прежние команды объектов и ожидающие замены отменяются; ручное отключение автоматизации, в том числе командой «Стой», снимается. Настроенные исходные скрипты выполняются с заданными длительностями, группы возвращаются к состояниям входа. Группы остаются остановленными: проверьте сцену и нажмите «Продолжить всё», чтобы запустить подготовку заново. Во время восстановления доступна кнопка «Остановить всё». Предметы, запасы магазинов и другие потраченные ресурсы не восстанавливаются автоматически.", "Click Restore initial state once. Previous object commands and pending replacements are cancelled; manual automation disables, including those set by Stop, are cleared. Configured initial scripts run with their set durations, and groups return to their entry states. Groups stay stopped: inspect the scene and select Resume all to start the prepared behavior afresh. Stop all remains available during restoration. Items, shop stock and other spent resources are not automatically restored.")
  ]),
