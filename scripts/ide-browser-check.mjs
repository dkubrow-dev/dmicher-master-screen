...
try {
  for (const version of ["13.351", "14.366"]) {
    ...
    assert.equal(await behavior.locator('[data-script-index] details[open]').count(),0);
    ...
    await behavior.locator('[name="field-name"]').fill('loudness'); await behavior.locator('[name="field-type"]').selectOption('integer');
    ...
    await app.locator(`[data-screen-action="selectNode"][data-id="${signalId}"]`).click();
    ...
    await clickTab('signals'); await app.locator(`[data-screen-action="selectNode"][data-id="${signalId}"]`).click();
    ...
    reports.push({version,checks:'layout, category overlays, native item forms, object info clipboard and native settings, initial/state/routine scripts, stable row IDs, multiple shops, signal schema and returns, FilePicker, cancelled state chooser',errors});
    await context.close();
  }
