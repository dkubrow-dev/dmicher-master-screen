...
function fixture() {
  ...
  globalThis.CONFIG = Object.fromEntries(["Actor", "Macro", "JournalEntry", "Scene"].map((type) => [type, { documentClass: {
    async create(data, options) {
      calls.push({ type, data: copy(data), options: copy(options) });
      const doc = document(type, `new${++count}`, copy(data));
      created.push(doc);
      return doc;
    }
  } }]));
...
test("scene export/import round trip keeps embedded IDs and remaps actors, macros and journal pages", async () => {
  ...
  assert.equal(sceneCall.data.flags[MODULE_ID].runtimes, undefined);
...
test("Actor compendium spawn is included and its exact UUID maps to the new world Actor", async () => {
  ...
  f.definition.episodes[2].spawns = [{ id: "reinforcement", actorUuid: compendium.uuid, x: 0, y: 0, count: 2, spacing: 100 }];
  const bundle = await exportBundle(f.scene);
  ...
  assert.equal(sceneData.flags[MODULE_ID].definitions.main.episodes[2].spawns[0].actorUuid, newActor.uuid);
});
...
test("native map notes bring their Journal and retain Page IDs under the new Journal", async () => {
  const f = fixture();
  f.definition.episodes[0].workspace.gm = [];
